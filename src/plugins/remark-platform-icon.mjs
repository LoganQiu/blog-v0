import { visit } from "unist-util-visit";

const appleSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
  <path d="M447.1 332.7C446.9 296 463.5 268.3 497.1 247.9C478.3 221 449.9 206.2 412.4 203.3C376.9 200.5 338.1 224 323.9 224C308.9 224 274.5 204.3 247.5 204.3C191.7 205.2 132.4 248.8 132.4 337.5C132.4 363.7 137.2 390.8 146.8 418.7C159.6 455.4 205.8 545.4 254 543.9C279.2 543.3 297 526 329.8 526C361.6 526 378.1 543.9 406.2 543.9C454.8 543.2 496.6 461.4 508.8 424.6C443.6 393.9 447.1 334.6 447.1 332.7zM390.5 168.5C417.8 136.1 415.3 106.6 414.5 96C390.4 97.4 362.5 112.4 346.6 130.9C329.1 150.7 318.8 175.2 321 202.8C347.1 204.8 370.9 191.4 390.5 168.5z"/>
</svg>`;

const windowsSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
  <path d="M96 96L310.6 96L310.6 310.6L96 310.6L96 96zM329.4 96L544 96L544 310.6L329.4 310.6L329.4 96zM96 329.4L310.6 329.4L310.6 544L96 544L96 329.4zM329.4 329.4L544 329.4L544 544L329.4 544L329.4 329.4z"/>
</svg>`;

export default function remarkPlatformIcons() {
  return (tree) => {
    visit(tree, "text", (node, index, parent) => {
      if (!parent || typeof index !== "number") return;

      let text = node.value;
      let modified = false;

      // 替换 :mac:
      if (text.includes(":mac:")) {
        text = text.replace(/:mac:/g, `<span class="icon mac">${appleSvg}</span>`);
        modified = true;
      }

      // 替换 :win:
      if (text.includes(":win:")) {
        text = text.replace(/:win:/g, `<span class="icon win">${windowsSvg}</span>`);
        modified = true;
      }

      // 如果有修改，替换为HTML节点
      if (modified) {
        parent.children[index] = {
          type: "html",
          value: text,
        };
      }
    });
  };
}
